...
local Library = { 
    Flags = { },
    MenuKeybind = tostring(Enum.KeyCode.RightControl), -- has to be a string
    ...
    local Themes = {
        ["Preset"] = {
            ...
            ["Accent"] = Color3.fromRGB(99, 99, 255),
    ...
    do
        ...
        Library.CreateKeybind = function(Self, Data)
            ...
            if Library.KeyList and Data.Name ~= "UI Keybind" then 
        ...
        Library.Watermark = function(Self, Params)
            ...
            function Watermark:Add(Text, Color)
                ...
                local TextColor = Color or "Accent"
                local ThemeData = type(TextColor) == "string" and {TextColor3 = TextColor} or nil
                ...
                local NewItem = Library:Create("TextLabel", {
                    ...
                    TextSize = Library.FontSize,
                    Parent = Items["Holder"].Instance,
                    TextColor3 = type(TextColor) == "string" and Library.Theme[TextColor] or TextColor,
                    Text = Text,
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    AutomaticSize = Enum.AutomaticSize.XY
        ...
        Library.Window = function(Self, Params)
            ...
            if Params.Watermark ~= false then
                local AutoWatermark = Library.Watermark(Window, {
                    Name = Params.WatermarkName or "siff."
                })

                local FpsText = AutoWatermark:Add("FPS: 0", Color3.fromRGB(255, 255, 255))
                local Success, ProductInfo = pcall(function()
                    return MarketplaceService:GetProductInfo(game.PlaceId)
                end)

                local GameName = Success and ProductInfo and ProductInfo.Name or Window.Name
                local GameNameText = AutoWatermark:Add(GameName, "Accent")
                ...
                Library:Connect(RunService.RenderStepped, function(DeltaT)
                    ...
                    if Elapsed >= 1 then
                        FpsText:SetText("FPS: " .. math.floor(FrameCount / Elapsed))
                        FrameCount = 0
                        Elapsed = 0
                    end
                end)

                Library.AutoWatermark = AutoWatermark
            end
        ...
        Library.List = function(Self, Params)
            ...
            local Items = { } do 
                Items["List"] = Library:Create("Frame", {
                    ...
                    Parent = Parent.Instance,
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1, 0, 0, Params.Height or Params.height or 200),
                    BorderSizePixel = 0
                })
        ...
        Library.Init = function(Self)
            local SettingsPage = Self:Page({Name = "settings"}) do 
                ...
                local function GetCurrentAutoloadName()
                    if not isfile(Library.Directory .. "/autoload.json") then
                        return "None"
                    end

                    local Name = readfile(Library.Directory .. "/autoload.json")
                    if Name == "" then
                        return "None"
                    end

                    if not isfile(ConfigsFolder .. Name .. ".json") then
                        writefile(Library.Directory .. "/autoload.json", "")
                        return "None"
                    end
                ...
                local function UpdateCurrentAutoloadLabel()
                    if CurrentAutoloadLabel then
                        CurrentAutoloadLabel:SetText("Current Autoload: " .. GetCurrentAutoloadName())
                    end
                end

                local MenuSection = SettingsPage:Section({Name = "UI", Side = 2}) do
                    MenuSection:Button({Name = "Unload", Callback = function()
                        Library:Exit()
                    end})

                    MenuSection:Label({ Name = "UI Keybind" }):Keybind({
                        Name = "UI Keybind",
                        Flag = "MenuKeybind",
                    ...
                    ConfigsSection:Button({
                        ...
                        Callback = function()
                            if ConfigSelected then 
                                ...
                                if isfile(Library.Directory .. "/autoload.json") and readfile(Library.Directory .. "/autoload.json") == ConfigSelected then
                                    writefile(Library.Directory .. "/autoload.json", "")
                                end

                                Library:GetConfigsList(ConfigsList)
                                UpdateCurrentAutoloadLabel()
                    ...
                    ConfigsSection:Button({
                        ...
                        Callback = function()
                            if ConfigSelected then
                                ...
                                writefile(Library.Directory .. "/autoload.json", ConfigSelected)
                                UpdateCurrentAutoloadLabel()
                                Library:Notification("Autoload set to " .. ConfigSelected, 3, Library.Theme.Accent)
                            end
                        end
                    })

                    CurrentAutoloadLabel = ConfigsSection:Label({
                        Name = "Current Autoload: " .. GetCurrentAutoloadName()
                    })
                ...
                local AutoloadConfig = readfile(Library.Directory .. "/autoload.json")

                if AutoloadConfig ~= "" then
                    local AutoloadPath = ConfigsFolder .. AutoloadConfig .. ".json"

                    if isfile(AutoloadPath) then
                        Library:LoadConfig(readfile(AutoloadPath))
                    else
                        writefile(Library.Directory .. "/autoload.json", "")
                        UpdateCurrentAutoloadLabel()
                    end
